...
export default function PreSaveLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await signIn("credentials", {
      email,
      fromPreSave: "true", // <-- Flag especial que criamos
      redirect: false,
    });
  ...
  return (
    <div className="fixed inset-0 bg-black overflow-hidden font-sans">
      ...
      <div className="relative z-10 w-full h-full overflow-y-auto overflow-x-hidden flex flex-col items-center justify-start pt-[15vh] md:justify-start md:pt-[5vh]">
        ...
        <img 
          src="/poesiadeboteco/INTRO.png?v=4" 
          alt="Acesso Poesia de Boteco" 
        ...
        <form onSubmit={handleSubmit} className="absolute inset-0 w-full h-full">
          ...
          <input
            ...
            className="absolute top-[62%] left-[32%] w-[58%] h-[8%] bg-transparent border-b-2 border-transparent hover:border-black/30 focus:border-black/50 text-black/80 font-bold text-[11px] sm:text-xs md:text-sm px-2 focus:outline-none transition-colors"
            placeholder=""
          />
          ...
          <button
            type="submit"
            disabled={loading}
            className="absolute top-[79.5%] left-[42.8%] w-[16%] h-[9%] bg-transparent hover:bg-black/10 active:bg-black/20 rounded-full cursor-pointer transition-colors focus:outline-none flex items-center justify-center disabled:opacity-50"
